...
/**
 * Generates metadata for the blog post page dynamically based on the slug.
...
export async function generateMetadata(props: Props) {
  ...
  if (post?.title) {
    return {
      ...
      openGraph: {
        ...
        description: post.metadata || "No description available",
  ...
  // Fallback metadata for when no post is found
  return {
    title: "Post Not Found",
    description: "No blog article has been found for this page.",
  };
}

/**
 * SingleBlog component renders a detailed view of a single blog post.
 * @param props - Component props containing the slug
 * @returns JSX element rendering the blog post details
 */
const SingleBlog = async (props: Props) => {
  ...
  // Index post data for Algolia if it exists
  if (post?.title) {
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative z-1 overflow-hidden pb-17.5 pt-35 lg:pb-22.5 xl:pb-27.5">
        ...
        {integrations.isSanityEnabled && post?.title ? (
            ...
            <div className="mx-auto mb-12.5 w-full max-w-[770px] text-center">
              <div className="mb-5 flex flex-wrap items-center justify-center gap-6">
                ...
                <Link
                  ...
                  className="flex items-center gap-2 font-satoshi font-medium -tracking-[0.2px] text-black hover:underline dark:text-gray-5"
                >
                  <svg
                  ...
                  {(post.author?.name || "")
                    .charAt(0)
                    .toUpperCase() + (post.author?.name || "").slice(1).toLowerCase()}
                </Link>
              ...
              <div className="mt-10 flex flex-wrap items-center justify-between gap-10 border-t border-stroke pt-9 dark:border-stroke-dark">
                <div className="flex items-center gap-5">
                  ...
                  <div className="w-full">
                    <Link
                      href={`/blog/author/${post.author?.slug?.current || ""}`}
                      className="block font-satoshi text-lg font-medium capitalize text-black hover:underline dark:text-white"
                    >
                      {post.author?.name || "Unknown Author"}
                    </Link>
                    <span className="block text-sm dark:text-gray-5">
